...
class ButtonComponent(name: String, private val delegate: BridgeDelegate<HotwireDestination>) :
    ...
    private fun handleButtonClick() {
        ...
        when {
            currentButtonTitle?.contains("Sign Out", ignoreCase = true) == true -> handleSignOut()
            currentButtonTitle?.contains("Print", ignoreCase = true) == true -> handlePrint()
        }
    }
    ...
    private fun handlePrint() {
        ...
        // Get the WebView from the navigator's session
        val navigator = delegate.destination.navigator
        ...
        val printAdapter = webView.createPrintDocumentAdapter("Headache Log")

        // Get the print manager
        val printManager = fragment.requireContext().getSystemService(PrintManager::class.java)

        // Create print job
        printManager?.print("Headache Log Print Job", printAdapter, null)
    }
